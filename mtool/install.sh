...
main() {
    ...
    local existing_id project_name=mtool existing_project='' existing_service=''
    existing_id=$(docker container inspect --format '{{.Id}}' mtool 2>/dev/null || true)
    ...
    if [[ -n "$existing_id" ]]; then
        printf '检测到已有 mtool 容器，将按本次输入重建。原挂载如下，请沿用需要保留的数据路径：\n' >&3
        docker inspect --format '{{range .Mounts}}{{println .Source "->" .Destination}}{{end}}' "$existing_id" >&3
    fi
    ask install_path 'MTool 安装目录（已有目录会备份配置后覆盖）' '/opt/mtool'
    ...
    if [[ -e "$install_path" ]]; then
        ...
        printf '检测到已有安装目录：将备份旧配置并覆盖，保留 data、logs、output 和媒体文件。\n' >&3
    fi
    ...
    exec 9> "$install_path/.mtool-install.lock"
    flock -n 9 || die '此目录已有安装进程在运行，请等它结束后重试。'
    mkdir -p -m 755 -- "$install_path/data" "$install_path/logs" "$install_path/output"
    local compose_file="$install_path/docker-compose.yml"
    MTOOL_STAGE=$(mktemp -d "$install_path/.mtool-stage.XXXXXX")
    local candidate_file="$MTOOL_STAGE/docker-compose.yml"
...
EOF
    ...
    if [[ "$existing_directory" == true || -n "$existing_id" || -e "$compose_file" || -L "$compose_file" ]]; then
        ...
        for name in docker-compose.yml docker-compose.yaml compose.yml compose.yaml .env; do
    ...
    mv -T -- "$candidate_file" "$compose_file"
    ...
    if [[ -n "$existing_id" && ( "$existing_project" != "$project_name" || "$existing_service" != mtool ) ]]; then
        # 非本项目 mtool 服务的旧容器不能由 Compose 接管；先正常停止，再移除容器本身。
        # 不使用 rm -v / down -v，不删除任何挂载卷或宿主机数据目录。
        docker stop "$existing_id" >/dev/null
        docker rm "$existing_id" >/dev/null
    fi
    if ! "${compose[@]}" up -d --force-recreate mtool; then
