...
| 宿主机 Web 端口 | `9808` |
| 容器内部端口 | 固定 `9808` |
| 监听地址 | `0.0.0.0`，监听全部 IPv4 网卡 |
| 用户名 | `admin` |
| 密码 | 隐藏输入；回车生成随机密码，手动密码至少 12 个字符 |
| 媒体挂载 | 只读；可在交互时启用写入及删除开关 |
| 特权模式 | 关闭；ISO loop 挂载可按需启用兼容模式 |

安装脚本默认监听宿主机全部 IPv4 网卡，可使用公网 IP 和宿主机 Web 端口访问；仍需配置云安全组、防火墙与 HTTPS。需要限制为本机访问时，在监听地址选项填写 `127.0.0.1`。密码登录不要长期通过公网明文 HTTP 使用。
...
脚本依次显示环境检查、部署参数、配置校验和镜像拉取、备份与容器重建、Web 就绪检查五个步骤。

Web 开始响应后会汇总监听地址、本机地址、公网 IPv4 访问地址、登录账号、配置路径、旧配置备份路径（如有），以及查看日志、重启服务和查看状态的命令。

公网 IPv4 优先通过 [ipify](https://www.ipify.org/) 查询，失败或返回内容异常时尝试 [Amazon checkip](https://checkip.amazonaws.com)。每个查询最多 3 秒，强制使用 IPv4 并绕过环境代理；只查询 IP，不发送账号、密码或部署配置。两个查询都失败时仍正常结束安装，并提示到 VPS 控制台查看公网 IPv4。

查询得到的是本机公网出口地址，不等于已从外部验证可达。云安全组、防火墙、NAT 和端口映射会影响实际访问；NAT VPS 对外端口可能与填写的宿主机端口不同。绑定 `127.0.0.1` 时不查询或展示公网地址。

## 通过 SSH 隧道访问

选择 `127.0.0.1` 监听时，在自己的电脑执行以下命令，将 SSH 端口、用户名、VPS 地址及 Web 端口替换为实际值：
...
脚本在发布前使用模拟 Docker/curl 验证首次安装与覆盖流程，包括旧配置备份、数据保留、当前容器端口复用、其他服务端口冲突、拉取失败及重建失败处理；还验证默认公网监听、IP 查询备用服务、非法响应、全部查询失败和本机监听时跳过公网查询。该检查不启动实际容器。实际镜像启动、认证、媒体处理和 ISO 挂载尚需在目标 VPS 上验证。脚本中的 Web 检查仅确认容器运行且 HTTP 有响应，不等于全部功能验收。
