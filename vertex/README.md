...
`8443` 在这里仍使用 **HTTP**，访问形式为 `http://公网IP:8443`。`0.0.0.0` 允许通过 VPS 的 IPv4 地址访问，还需要放行安全组和防火墙端口；NAT VPS 需要相应的外部端口映射。选择 `127.0.0.1` 时仅本机可直接访问。
...
沿用配置的范围如上；镜像和启动命令统一采用本脚本配置，日志采用有大小限制的 Docker `local` 驱动。自定义 entrypoint、额外 capabilities、设备、特权等高级容器参数不会自动复制；使用这些参数的部署应先检查原 Compose 配置。
